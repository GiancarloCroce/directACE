...
// compute log(Z) and its derivatives
void optimizeF(const std::vector<double> &J, double &F, std::vector<double> &grad, int length, double &b) {


    int spins[length];
    long double expJ[J.size()];
    long double Z=0;

    // Calculate Z

    int i;
    for (i=0;i<length;i++) { spins[i]=0; expJ[i] = exp(-b*J[i]); grad[i]=0; }
    for (i=length;i<J.size();i++)     {  expJ[i] = exp(-b*J[i]); grad[i]=0;}
    ...
    for (i=0;i<twoToLength;i++) { 
        ...
        for (j=0;j<lastGrad;j++)    grad[foundGrad[j]] += temp;
